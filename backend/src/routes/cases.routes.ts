 ...
 */
router.post('/', authenticate, uploadCasePhotos, async (req: AuthRequest, res) => {
  try {
    ...
    await intakeQueue.add('process-case', {
      case_id: newCase.case_id,
      urgency: newCase.urgency
    }, {
      priority: newCase.urgency === 'P1' ? 1 : (newCase.urgency === 'P2' ? 5 : 10)
    });
...
router.patch('/:case_id', authenticate, async (req, res) => {
  try {
    ...
    const updatedCase = await caseService.updateCase(req.params.case_id, updates);
...
router.post('/:case_id/still-searching', authenticate, async (req, res) => {
  try {
    ...
    const updatedCase = await caseService.extendBroadcast(req.params.case_id, minutes);
...
router.get('/zone/:zone_id', authenticate, async (req, res) => {
  try {
    const cases = await caseService.getCasesByZone(req.params.zone_id, 'active');
