...
> = {
  ...
  "ZONE-3": { lat: 20.0, lng: 73.785, name: "Zone 3 - Panchavati" },
  "ZONE-4": { lat: 20.0015, lng: 73.787, name: "Zone 4 - Exit Point" },
};
  ...
  private async simulatePersonMovement(person_id: string) {
    ...
    const moveToNextZone = async () => {
      ...
      if (currentZoneIndex < ZONE_SEQUENCE.length - 1) {
        ...
        // Move person
        const { person: updatedPerson, movement } =
          await personTrackingService.movePerson(
            person_id,
            nextZoneId,
            nextZone.lat,
            nextZone.lng,
        ...
        if (io) {
          ...
          io.to(`zone:${person.current_zone}`).emit(
            "person:left",
            movementEvent,
          );
        ...
        console.log(
          `🚶 ${person.name} moved: ${person.current_zone} → ${nextZoneId}`,
        );
        ...
        console.log(
          `✅ ${person.name} completed journey at ${person.current_zone}`,
        );
      }
    };
