...
const caseTypes = [
  {
    ...
    items: ['Reporter + Lost Person Photos', 'GPS Location Auto-capture', 'Physical Description & Clothing'],
  },
  {
    ...
    items: ['Reporter Photo', 'Upload or Describe Person', 'Cross-center Match Search'],
  },
  {
    id: 'found' as const,
    title: 'Found Person',
    ...
    items: ['Reporter + Found Person Photos', 'GPS Location', 'Auto P1 Alert to Police'],
  },
];
      ...
      <header className="bg-gray-900 border-b border-gray-800">
        <div className="max-w-4xl mx-auto px-4 py-3 flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-kumbh-orange to-kumbh-gold flex items-center justify-center text-sm font-black shadow">K</div>
            <div>
              <h1 className="text-sm font-bold">Sherlock Station</h1>
              <p className="text-[10px] text-gray-500">{user?.name} · Field Volunteer</p>
            </div>
          </div>
          <button onClick={logout} className="flex items-center gap-1.5 text-xs text-gray-500 hover:text-gray-300 px-3 py-1.5 rounded-lg hover:bg-gray-800 transition">
        ...
        {!selectedCase ? (
          <div>
            <div className="text-center mb-10">
              <h2 className="text-2xl font-bold mb-2">What do you need to report?</h2>
              <p className="text-sm text-gray-500">Select the type of case to file a report</p>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-4">
              {caseTypes.map((c) => (
                ...
                >
                  <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${c.color} flex items-center justify-center text-white mb-4 shadow-lg group-hover:scale-110 transition-transform`}>
                    {c.icon}
                  </div>
                  ...
                  <div className={`mt-4 inline-block text-[10px] font-semibold uppercase tracking-wider px-2 py-1 rounded-md ${c.badge}`}>
                    {c.id === 'found' ? 'Auto P1 Escalation' : c.id === 'lost' ? 'Broadcast Alert' : 'Cross-Center Match'}
                  </div>
                </button>
              ))}
            </div>
