...
function HeatmapLayer({ points }: { points: HeatPoint[] }) {
  ...
  useEffect(() => {
    ...
    const heat = (L as any).heatLayer(
      validPoints.map((p) => [p.lat, p.lng, p.intensity || 0.5]),
      {
        radius: 25,
        blur: 15,
        maxZoom: 17,
...
export default function InfrastructureMap({
  ...
  return (
    <div
      style={{ height }}
      className="rounded-xl overflow-hidden border-2 border-gray-600 shadow-2xl"
    >
      <MapContainer
        center={center}
        zoom={12}
      ...
      >
        ...
        {layers.policeStations &&
          policeStations.map((ps) => (
            <Marker
              key={ps.station_id}
              position={[ps.gps_lat, ps.gps_lng]}
              icon={policeIcon}
            >
              <Popup>
          ...
          chokepoints.map((cp) => (
            ...
            >
              <Popup>
                <div className="text-xs">
                  <strong>
                    {chokepointTypeEmoji[cp.chokepoint_type] || "📍"}{" "}
                    {cp.chokepoint_name}
                  </strong>
                  <br />
                  ...
                  <br />
                  Risk:{" "}
                  <span style={{ color: riskColors[cp.risk_level] }}>
                    {cp.risk_level}
                  </span>
                </div>
              </Popup>
